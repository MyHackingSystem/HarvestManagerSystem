...
namespace HarvestManagerSystem.database
{
    class RapportDAO:DAO
    {
        ...
        //*******************************
        //Company Harvest Quantity Rapport
        //*******************************
        public List<Production> CompanyHarvestQuantity(DateTime fromDate, DateTime toDate, int type, int supplierId)
        {
        ...
        //*******************************
        //Company Harvest Hours Rapport
        //*******************************
        public List<Production> CompanyHarvestHours(DateTime fromDate, DateTime toDate, int type, int supplierId)
        {
            string select = "SELECT "
        + ProductionDAO.TABLE_PRODUCTION + "." + ProductionDAO.COLUMN_PRODUCTION_ID + ", "
        + ProductionDAO.TABLE_PRODUCTION + "." + ProductionDAO.COLUMN_PRODUCTION_TYPE + ", "
        ...
        //*******************************
        //Employee Harvest Hours Rapport
        //*******************************
        public List<HarvestHours> EmployeeHarvestHours(DateTime fromDate, DateTime toDate, int employeeId)
        {
            ...
            try
            {
                ...
                OpenConnection();
                SQLiteDataReader result = sQLiteCommand.ExecuteReader();
                return getHarvestHoursFromResult(result);
            }
            catch (SQLiteException e)
            {
                Console.WriteLine(e.StackTrace);
        ...
        public List<HarvestHours> getHarvestHoursFromResult(SQLiteDataReader result)
        {
            ...
            if (result.HasRows)
            {
                while (result.Read())
                {
                    ...
                    harvestHours.HarvestHoursID = Convert.ToInt32((result[HarvestHoursDAO.COLUMN_HOURS_ID]).ToString());
                    ...
                    harvestHours.Remarque = Convert.ToString(result[HarvestHoursDAO.COLUMN_HOURS_REMARQUE]);
        ...
        //***********************************
        //Employee Harvest Quantity Rapport
        //***********************************
        public List<HarvestQuantity> EmployeeHarvestQuantity(DateTime fromDate, DateTime toDate, int employeeId)
        {
            string selectStmt = "SELECT "
                ...
                + " LEFT JOIN " + ProductDetailDAO.TABLE_PRODUCT_DETAIL + " "
                + " ON " + ProductDetailDAO.TABLE_PRODUCT_DETAIL + "." + ProductDetailDAO.COLUMN_PRODUCT_DETAIL_ID + " = " + ProductionDAO.TABLE_PRODUCTION + "." + ProductionDAO.COLUMN_PRODUCTION_PRODUCT_DETAIL_ID
                + " WHERE " + HarvestQuantityDAO.COLUMN_QUANTITY_DATE
                + " BETWEEN strftime('%Y-%m-%d %H:%M:%S', '" + fromDate.ToString("yyyy'-'MM'-'dd' 'HH':'mm':'ss") + "') AND strftime('%Y-%m-%d %H:%M:%S', '" + toDate.ToString("yyyy'-'MM'-'dd' 'HH':'mm':'ss") + "') "
                + " AND " + EmployeeDAO.TABLE_EMPLOYEE + "." + EmployeeDAO.COLUMN_EMPLOYEE_ID + " = " + employeeId
            ...
            try
            {
                ...
                OpenConnection();
                SQLiteDataReader result = sQLiteCommand.ExecuteReader();
                return getHarvestQuantityFromResult(result);
            }
            catch (SQLiteException e)
            {
                Console.WriteLine(e.StackTrace);
        ...
        public List<HarvestQuantity> getHarvestQuantityFromResult(SQLiteDataReader result)
        {
            ...
            if (result.HasRows)
            {
                while (result.Read())
                {
                    ...
                    item.HarvestQuantityId = Convert.ToInt32((result[HarvestQuantityDAO.COLUMN_QUANTITY_ID]).ToString());
                    ...
                    item.Remarque = Convert.ToString(result[HarvestQuantityDAO.COLUMN_QUANTITY_REMARQUE]);
